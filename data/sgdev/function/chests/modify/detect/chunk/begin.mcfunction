execute if score $Debug sgdev.var matches 2.. run say sgdev:chests/modify/detect/chunk/begin
...
execute if score $ChunkIndex sgdev.array matches ..-1 run return run function sgdev:chests/modify/output/error
...
execute store result score $ChunkOutput sgdev.array run function sgdev:chests/modify/detect/chunk/check_at_index with storage sgdev:array

#sgdev:array ChunkIndex is still the correct index
#if $ChunkOutput = -1, display error
execute if score $ChunkOutput sgdev.array matches -1 run function sgdev:chests/modify/output/error
#if $ChunkOutput != -1, loop through the chunk's chests
execute unless score $ChunkOutput sgdev.array matches -1 run function sgdev:chests/modify/detect/chest/begin with storage sgdev:array